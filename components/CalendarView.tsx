            ...
            <div className="max-h-48 overflow-y-auto p-1.5 no-scrollbar">
              {CITIES.map((city) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 hover:bg-white/5 rounded-lg text-[11px] transition-all ${location.name === city.name ? 'text-blue-400 bg-blue-500/5 font-bold' : 'text-gray-400'}`}
                >
                  <span>{city.name}</span>
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <button 
                  ...
                  className="hidden md:block absolute top-1 right-1 opacity-0 group-hover:opacity-100 p-1.5 hover:bg-white/10 rounded-full text-gray-500 hover:text-white transition-all z-20"
                >
                  <Plus size={16} />
                </button>
              </div>
                  ...
                  .map(event => {
                    ...
                    return (
                      <div
                        key={event.id}
                        onClick={(e) => { e.stopPropagation(); handleEventClick(event); }}
                        className={`absolute left-1 right-1 rounded-lg p-1.5 text-[10px] overflow-hidden cursor-pointer hover:brightness-110 hover:shadow-lg hover:z-20 transition-all shadow-sm z-10 ${event.color || 'bg-blue-600'} text-white border border-white/10 flex flex-col`}
                        style={{
                ...
                .map(event => {
                  ...
                  return (
                    <div
                      key={event.id}
                      onClick={(e) => { e.stopPropagation(); handleEventClick(event); }}
                      className={`absolute left-4 right-4 rounded-xl p-4 text-sm overflow-hidden cursor-pointer hover:brightness-110 hover:shadow-xl hover:z-20 transition-all shadow-md z-10 ${event.color || 'bg-blue-600'} text-white border border-white/10 flex flex-col gap-1`}
                      style={{
            ...
            <div className={`h-24 relative flex items-end p-6 ${selectedHoliday.isPublic ? 'bg-gradient-to-br from-red-600/30 to-red-900/50' : 'bg-gradient-to-br from-emerald-600/30 to-emerald-900/50'}`}>
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-1.5 py-0.5 bg-white/10 rounded-full text-[8px] font-black uppercase tracking-widest text-white/80">
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto no-scrollbar">
              <div className="flex items-center gap-4">
                <div className="flex flex-col items-center justify-center w-12 h-12 bg-white/[0.03] border border-white/5 rounded-xl">
                  <span className="text-[8px] font-bold text-gray-500 uppercase">{new Date(selectedHoliday.date).toLocaleDateString('ro-RO', { month: 'short' })}</span>
                  <span className="text-lg font-light text-gray-200">{new Date(selectedHoliday.date).getDate()}</span>
                </div>
                <div className="flex-1">
                  <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Data</div>
              ...
              <div className="space-y-2">
                <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Descriere</div>
